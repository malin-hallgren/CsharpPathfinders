...
namespace PathfinderTest
{
    ...
    //        WritePath(pathTaken, goal, new Node(1, 1));
    //    }

    //    static private void WritePath((int, int)[] path, Node goal, Node start)
    //    {
    ...
    //        Console.WriteLine($"Goal reached in {path.Length} steps\nPath taken {string.Join("->", path)}"); //StringBuilder?
    //    }
    ...
    //    static private int CalculateH(int row, int col, Node goal)
    //    {
    //        //return Math.Sqrt(Math.Pow(row - goal.Row, 2) + Math.Pow(col - goal.Col, 2));
    //        int dx = Math.Abs(row - goal.Row);
    //        int dy = Math.Abs(col - goal.Col);
    //        int baseCost = 1;
    //        int diagCost = baseCost * 2;
    //        return baseCost * (dx + dy) + (diagCost - 2 * baseCost) * Math.Min(dx, dy);
    //    }
    ...
    //                        {
    //                            double gNew = cells[row, col].g + 1; //when breaking out, use base or diag cost here, suggested by copilot: gNew = cells[row,col].g + (i!=0 && j!=0 ? diagCost : 1)
